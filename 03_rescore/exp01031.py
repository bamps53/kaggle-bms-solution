...
class Config:
    ...
    # candidate_gcs_dir: str = GCS_PATHS['416x736_no_pad_sp_denoise_candidates']
    candidate_gcs_dir: str = GCS_PATHS['416x736_no_pad_sp_denoise_yokoo0601']
...
class Tokenizer(object):
    ...
    def tokenize(self, inchi, max_len=277):
        ...
        try:
            x = np.array(tokenizer.text_to_sequence(inchi), np.int64)
            ...
            # return np.ones(max_len, dtype=np.int64) * self.stoi['<pad>']
...
def get_test_dataset(df_path, batch_size=32):
    ...
    file_paths = df['image_id'].map(
        lambda x: f'../input/resized416x736_no_pad_sp_denoise_test/{x}.png').tolist()

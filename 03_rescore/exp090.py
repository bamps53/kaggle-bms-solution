...
@dataclass
class CFG:
    exp_id = 'exp090_300_600_en_de_transformer_b4_2layers_seqlen200_fix_pos_random_crop_focal_pre_norm'
    debug: bool = False
    ...
    image_width: int = 600
    row_size: int = image_height // 32 + 1
    col_size: int = image_width // 32 + 1

    # dtype: str = tf.bfloat16 if TPU else tf.float32
    dtype: str = tf.float32
    train_gcs_dir: str = GCS_PATHS['300x600_no_pad']
    val_gcs_dir: str = GCS_PATHS['300x600_no_pad']
    test_gcs_dir: str = GCS_PATHS['300x600_no_pad_test']
    val_size: int = 121210
...
def read_image(file_name, label, height=300, width=600, seq_len=200):
...
def get_test_dataset(df_path, batch_size=32):
    ...
    file_paths = df['image_id'].map(
        lambda x: f'../input/test_resized300x600_no_pad/{x}.png').tolist()
...
class Encoder(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, drop_rate):

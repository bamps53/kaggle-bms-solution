...
class Config:
    # general
    exp_id = 'exp103_416x736_transformer_b4_2layers_seqlen200_salt_pepper'
    ...
    def __init__(self, encoder, decoder, optimizer, scheduler, loss_fn, metric_fn, num_epochs=100, resume=False, resume_epoch=0, steps_per_epoch=None):
        ...
        if self.resume:
            if CFG.resume_from_last:
                ...
                last_step = get_last_step(CFG.save_dir)
                if last_step > 0:
...
with strategy.scope():
    ...
    init_lr = 1.0e-3 * CFG.batch_size_base / 32
    last_lr = 1.0e-5 * CFG.batch_size_base / 32

...
class Config:
    # general
    exp_id = 'exp1031_416x736_transformer_b4_2layers_seqlen200_finetune_with_noise_denoise'
    ...
    resume_from: str = 'exp103_416x736_transformer_b4_2layers_seqlen200_salt_pepper'
    ...
    train_gcs_dir: str = GCS_PATHS['416x736_no_pad_noise_denoise']
    val_gcs_dir: str = GCS_PATHS['416x736_no_pad_noise_denoise']
    test_gcs_dir: str = GCS_PATHS['416x736_no_pad_noise_denoise_test']
    val_size: int = 121210
...
def read_tfrecord(example):
    ...
    img = tf.reshape(img, (CFG.image_height, CFG.image_width))
...
def salt_and_pepper(img, labels):
    pepper_amount = tf.random.uniform([], 0, 0.001, tf.float32)
    pepper = tf.random.uniform(
        [CFG.image_height, CFG.image_width], 0, 1, tf.float32) < pepper_amount
    img = tf.where(pepper[:, :, None], tf.zeros_like(img), img)

    salt_amount = tf.random.uniform([], 0, 0.3, tf.float32)
    salt = tf.random.uniform(
        [CFG.image_height, CFG.image_width], 0, 1, tf.float32) < salt_amount
...
def get_dataset(mode, batch_size=64, data_root='./', fold=0):
    ...
    if mode == 'train':
        ...
        # ds = ds.map(salt_and_pepper, num_parallel_calls=AUTO)
...
class FocalLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        with tf.name_scope('focal_loss'):
            is_pad = tf.not_equal(y_true, PAD_TOKEN)
            is_sos = tf.not_equal(y_true, START_TOKEN)
...
class Trainer:
    def __init__(self, encoder, decoder, optimizer, scheduler, loss_fn, metric_fn, num_epochs=100, resume=False, resume_epoch=0, steps_per_epoch=None):
        ...
        if self.resume:
            if CFG.resume_from:
                save_dir = f'gs://model_storage53/{CFG.resume_from}'
                last_step = int(tf.io.gfile.glob(
                    f'{save_dir}/encoder_step*')[-1].strip('').split('step')[1][:6])
                self.init_epoch = self.total_steps // CFG.steps_per_epoch + 1
    ...
    def train_step(self, images, labels):
        ...
        with tf.GradientTape() as tape:
            ...
            is_pad = tf.not_equal(labels_target, PAD_TOKEN)
            is_sos = tf.not_equal(labels_target, START_TOKEN)
            mask = tf.logical_or(is_pad, is_sos)
...
with strategy.scope():
    ...
    init_lr = 1.0e-4 * CFG.batch_size_base / 32
    last_lr = 1.0e-6 * CFG.batch_size_base / 32
    lr_fn = [lrfn(step, 1e-6, init_lr, last_lr, CFG.total_steps)
